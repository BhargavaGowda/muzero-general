...
class Connect4:
    ...
    def get_observation(self):
        board_player1Hidden = numpy.zeros(self.board.shape)
        board_player2Hidden = numpy.zeros(self.board.shape)
        for i in range(self.board.shape[1]):
            if(self.board[5,i] != 0):
                board_player1Hidden[:,i] = numpy.where(self.board[:,i]==1, 1.0, 0.0)
                board_player2Hidden[:,i] = numpy.where(self.board[:,i]==-1, 1.0, 0.0)            
        board_to_play = numpy.full((6, 7), self.player, dtype="int32")
        return numpy.array([board_player1Hidden, board_player2Hidden, board_to_play])
    ...
    def render(self):
        ...
        for i in range(self.board.shape[1]):
            if(self.board[5,i] != 0):
                ...
                board_player2Hidden[:,i] = numpy.where(self.board[:,i]==-1, 1.0, 0.0) 
        print(self.board[::-1])
        print(board_player1Hidden[::-1])
        print(board_player2Hidden[::-1])
